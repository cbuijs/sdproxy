...
Version: 1.13.0
Updated: 10-Jun-2026 10:30 CEST
...
Changes:
  ...
  1.11.0 - [CODE SMELL/FIX] Introduced `ErrSilentDrop` strongly-typed sentinel 
           error natively. Eradicates brittle string-based error matching 
           within the SingleFlight consensus validation drops.
  1.10.0 - [SECURITY/FIX] Completely eradicated a catastrophic Cache Contamination 
           and SingleFlight Collision vulnerability within `buildSFKey` natively. 
           Unconditionally appended null-byte (`\x00`) boundary delimiters for all 
           optional string parameters (`clientName`, `ecs`). Neutralizes the exploit 
           where parallel clients generating opposing empty/non-empty states 
           erroneously hijacked coalesced locks, delivering subnet-optimized 
           downstream payloads to the wrong origin client organically.
*/
...
func buildSFKey(name string, qtype uint16, qclass uint16, routeIdx uint16, doBit bool, cdBit bool, clientName string, ecs string) string {
