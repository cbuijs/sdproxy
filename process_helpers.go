...
Changes:
  1.18.0 - [SECURITY/FIX] Resolved uint16 parsing regression within `extractIPFromPTR`.
           ...
           construct natively, causing out-of-bounds panics on malformed requests.
           Replaced with exact bitwise operations organically.
...
func extractIPFromPTR(name string) string {
	if strings.HasSuffix(name, ".in-addr.arpa") {
		...
		for i := 0; i < 16; i++ {
			...
			// [SECURITY/FIX] Cast to byte before shifting to prevent index/bounds panics natively.
