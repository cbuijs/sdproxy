...
Changes:
  1.7.0 - [REFACTOR] Applied `PreserveEDNS0` and `RcodeStr` utilities dynamically 
          to deduplicate telemetry bounds securely.
  1.6.0 - [LOGGING] Bound native DDR intercepts explicitly to `logDDR` 
          configurations rather than generic `logQueries` organically.
*/
...
func handleDDR(w dns.ResponseWriter, r *dns.Msg, q dns.Question, qNameTrimmed, clientID, protocol string) bool {
	...
	*resp         = dns.Msg{} // Zero fields safely
	...
	if logDDR {
		...
		log.Printf("[DDR] [%s] %s -> %s %s | %s | %s", protocol, clientID, q.Name, dns.TypeToString[q.Qtype], label, rcodeStr)
	}
	msgPool.Put(resp)
	return true
}
