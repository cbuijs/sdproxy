...
Changes:
  ...
  1.13.0 - [BUG/FIX] Resolved a severe Penalty Box initialization bug. Correctly 
           persists the default `MaxTrackedIPs` (100000) back to the global `cfg` 
           struct. Prior to this, undefined config entries evaluated to 0, 
           preventing the fast-path from registering any blackholed clients.
  1.12.0 - [SECURITY/FIX] Resolved an IPv4-in-IPv6 Translation vulnerability natively. 
           `groupIPFast` now explicitly evaluates the `addr.String()` payload during 
           maximum boundary thresholds (e.g., `/32` and `/128`) instead of the unmapped 
           `ipStr`. This ensures Protocol Abstraction Leaks (TCP/DoH transmitting `::ffff:X.X`) 
           do not dynamically bypass Instant-Ban penalty constraints established by 
           the HTTP server logic natively.
...
func InitRateLimiter() {
	...
	cfg.Server.RateLimit.MaxTrackedIPs = maxIPs // BUG FIX: update the global config struct!
