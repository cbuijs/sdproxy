...
Version: 2.46.0
Updated: 08-Jun-2026 14:59 CEST
...
Changes:
  ...
  2.43.0 - [ROBUSTNESS] Dynamically injected `KeepAlivePeriod` and `MaxIdleTimeout` 
           directly into HTTP/3 (QUIC) configurations natively. Protects DoH3 
           upgrades from encountering severe NAT timeouts or `H3 error 0x5` drops 
           by persistently polling upstream servers with background PING frames.
  2.42.0 - [LOGGING] Guarded `ECH`, `DDR`, `UPGRADE`, and Bootstrap evaluations 
           tightly behind their respective configured boundaries natively.
*/
...
func ParseUpstream(raw string, bootstrapNodes []*Upstream) (*Upstream, error) {
	...
	if needsDDR && (u.Proto == "dot" || u.Proto == "doh" || u.Proto == "doh3" || u.Proto == "doq") {
		...
		if resolveHost != "" && len(bootstrapNodes) > 0 {
			echConfig, hIPs, h3Supported := bootstrapResolveECH(resolveHost, bootstrapNodes)
