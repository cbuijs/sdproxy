...
// decodeBase64Flex tests multiple standard encodings to successfully extract ECH payloads natively.
func decodeBase64Flex(s string) ([]byte, error) {
	if d, err := base64.RawURLEncoding.DecodeString(s); err == nil { return d, nil }
	if d, err := base64.URLEncoding.DecodeString(s); err == nil { return d, nil }
	if d, err := base64.StdEncoding.DecodeString(s); err == nil { return d, nil }
...
func ParseUpstream(raw string, bootstrapNodes []*Upstream) (*Upstream, error) {
	...
	if echBase64 != "" {
		if useECH == "disable" {
			...
			decoded, err := decodeBase64Flex(echBase64)
			if err == nil {
