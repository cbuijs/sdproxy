...
Changes:
  ...
  3.1.0  - [FEAT] Persisted the new per-client last-seen tracker
           ...
           ResetStats. Without this a daemon restart would blank the Web UI
...
type SavedStats struct {
	...
	// ClientLastSeen maps a client identity (source IP string, or MAC address
	// string) to the unix-second instant at which it last presented an admitted
	// query. Flat rather than hourly-binned: see the 3.1.0 note above.
	// Absent in files written before 3.1.0; decodes as nil, which Import treats
	// as a no-op.
	ClientLastSeen map[string]int64 `json:"client_last_seen"`
}
...
func ResetStats() {
	...
	// The Last Seen column is part of what the dashboard displays, so a
	// statistics reset must clear it too — leaving stale ages behind after an
	// explicit "reset everything" would be actively misleading.
	statClientSeen.Clear()
...
func LoadStats() {
	...
	if data.ClientLastSeen != nil {
		// Import applies both the age horizon (clientSeenMaxAge) and the entry
		// ceiling internally, so an oversized or ancient payload is clamped
		// here rather than trusted.
		statClientSeen.Import(data.ClientLastSeen)
	}
