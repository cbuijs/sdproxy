...
Changes:
  2.56.0 - [FEAT] Propagated `bypassGlobal` to filtering helpers within the exchange context.
...
func executeUpstreamExchange(
	...
	bypassPolicies bool, bypassGlobal bool,
	...
	sfKey       := buildSFKey(qNameTrimmed, q.Qtype, q.Qclass, routeIdx, doBit, r.CheckingDisabled, bypassGlobal, sfClientName, cacheKey.ECS)
	didUpstream := false
	...
	if checkTargetNames(w, r, finalResp, sk, clientGroup, clientMAC, clientIP, clientAddr, clientName, clientNameLower, clientID, protocol, sni, sniLower, path, pathLower, bypassPolicies, bypassGlobal, originalQName) {
		return
	}
	...
	if filterResponseIPs(w, r, finalResp, sk, clientGroup, clientMAC, clientIP, clientAddr, clientName, clientNameLower, clientID, protocol, sni, sniLower, path, pathLower, bypassPolicies, bypassGlobal, originalQName, originalQNameTrimmed) {
		return
	}
