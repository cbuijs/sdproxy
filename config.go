...
func (c *CategoryConfig) UnmarshalYAML(value *yaml.Node) error {
	...
	*c = CategoryConfig(w.plain)
	...
	case w.Source.Kind == yaml.SequenceNode:
		// Do not replace c.ListURLs completely if we decode directly into it,
		// decode appends. Since we construct a fresh CategoryConfig, this is fine.
		var list []string
		if err := w.Source.Decode(&list); err != nil {
...
func validateConfig() error {
	...
	if len(cfg.Groups) > 0 {
		...
		// [FIX] Initialize cfg.Groups if it was nil. This prevents map assignment
		// panics later in the pipeline if other routines assume it is non-nil.
	...
	if len(cfg.Upstreams) > 0 {
		...
		cfg.Upstreams = make(map[string]UpstreamGroupConfig)
	}
	...
	if cfg.Routes == nil { cfg.Routes = make(map[string]RouteConfig) }
	...
	if cfg.RoutesFiles == nil { cfg.RoutesFiles = make(map[string]RouteConfig) }
	...
	if cfg.DomainRoutes == nil { cfg.DomainRoutes = make(map[string]DomainRouteConfig) }
	...
	if cfg.DomainRoutesFiles == nil { cfg.DomainRoutesFiles = make(map[string]DomainRouteConfig) }
