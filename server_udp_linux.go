...
// Changes:
//   1.5.0 - [SECURITY/FIX] handleUDPDirect performed a bare type assertion to
//           *net.UDPAddr and left the client IP empty on failure, producing an
//           unidentifiable origin. Combined with the fail-open admission gate
//           that existed prior to process_security.go 1.15.0, that meant such a
//           query bypassed both the ACL and the rate limiter. Now uses the
//           shared, type-agnostic remoteIPFromAddr() helper (server.go 1.36.0).
...
func handleUDPDirect(w dns.ResponseWriter, r *dns.Msg) {
    // [SECURITY 1.5.0] Type-agnostic extraction. A failed assertion no longer
    // silently degrades the peer into an unidentifiable (and, before
    // process_security.go 1.15.0, entirely unpoliced) origin.
