...
Version: 2.11.0 (Split)
Updated: 18-Jun-2026 09:59 CEST
...
Changes:
  2.11.0 - [PERF] Migrated persistent cache architecture to Binary Serialization (`encoding/gob`).
           Eradicates catastrophic CPU parsing overhead during cold boots by 
           directly loading structural memory maps instead of decoding raw JSON strings.
  2.10.0 - [SECURITY/FIX] Upgraded `saveCatCache` to rigidly assess `os.WriteFile` constraints. 
           Eradicates potential cache contamination via corrupted 0-byte file renames natively.
...
func loadCatCache() {
	pathBin := filepath.Join(snapshotDir(), "parental-catcache.bin")
	var data catCacheData
	loadedBin := false

	// Attempt ultra-fast binary loading first natively
	if f, err := os.Open(pathBin); err == nil {
		br := bufio.NewReaderSize(f, 64*1024)
		if err := gob.NewDecoder(br).Decode(&data); err == nil {
			loadedBin = true
		} else {
			if logParental {
				log.Printf("[PARENTAL] Binary cache decode error: %v", err)
			}
		}
		f.Close()
	}

	// Fallback to legacy JSON format for seamless version upgrades organically
	if !loadedBin {
	...
	if logParental {
		if loadedBin {
			log.Printf("[PARENTAL] Binary category cache loaded instantly: %d apex, %d patterns, %d IPs, %d CIDRs", len(data.Apex), len(patterns), len(ips), len(cidrs))
		} else {
			log.Printf("[PARENTAL] JSON Category cache loaded: %d apex, %d patterns, %d IPs, %d CIDRs", len(data.Apex), len(patterns), len(ips), len(cidrs))
		}
	}
}

func saveCatCache(apex map[string]string, patterns []wildcardCatEntry, ips map[netip.Addr]string, cidrs []cidrCatEntry) {
	path := filepath.Join(snapshotDir(), "parental-catcache.bin")
	...
	err = gob.NewEncoder(bw).Encode(data)
	if err == nil {
		bw.Flush()
		f.Sync()
		f.Close()
		os.Rename(tmp, path)
		// Prune obsolete JSON payload if it exists natively
