...
Changes:
  1.242.0 - [SECURITY/FIX] Added the InitRecentBlocks() call that starts the
            ...
            previously launched from InitCache, which returns early when the
            cache is disabled — so `cache.enabled: false` silently left the
            tracker growing unbounded (see process_leak.go 1.4.0, cache.go
            2.49.0). Placement is deliberate: it sits immediately after
            `searchDomainLeakPrevention` is resolved from config, since
...
func main() {
	...
	if cfg.Server.SearchDomainLeakPrevention != nil {
		...
		searchDomainLeakPrevention = true
	}

	// [SECURITY/FIX 1.242.0] Must follow the flag resolution directly above:
	// InitRecentBlocks() consults searchDomainLeakPrevention to decide whether
	// the reclaimer goroutine is needed at all. Previously this launch was
	// buried inside InitCache, which both runs too early to see this flag and
	// short-circuits entirely when caching is disabled — stranding the tracker
	// without a reclaimer in exactly that configuration.
	InitRecentBlocks()
