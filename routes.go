...
Changes:
  ...
  1.3.0 - [PERF] Employs zero-allocation fast-glob matching natively instead 
          ...
          the hot-path routing evaluations.
...
func classifyRouteKey(key string) routeKeyType {
	...
	if strings.HasPrefix(strings.ToUpper(key), "CC:") && len(key) == 5 {
		return rkCountry
	}
...
// [PERF] Employs zero-allocation fast-glob matching natively instead of path.Match.
// Eradicates heap escapes and parsing overhead on the hot-path routing evaluations.
