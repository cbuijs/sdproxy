...
Changes:
  ...
  1.3.0 - [CLEANUP] Synchronized `filterRebinding` call natively with the newly 
          streamlined, dead-code-free filtering signature.
  1.2.0 - [SECURITY/FIX] Added RecursionDesired to the SingleFlight key.
          [SECURITY/FIX] Resolved a severe cache payload modification vulnerability natively.
...
func (qc *queryCtx) serveFromUpstream() {
	...
	if filterRebinding(qc.w, qc.r, finalResp, qc.clientIP, qc.clientID,
	...
	// [PERF 1.4.0] Inlined `responseContainsNullIP` evaluation natively.
	isNullIP := false
