...
## Unreleased — v1.449.0 (07-Aug-2026)

### Web UI — client "Last Seen"
- **[FEAT]** New `stats_lastseen.go` (1.0.0): sharded (32×), bounded per-client
  last-seen tracker. Hot path is one RWMutex read-lock + one map read + one
  atomic store, zero allocations; the write lock is taken only for a new client.
- **[FEAT]** `process_security.go` 1.16.0 — `TouchClientSeen()` stamped once per
  admitted query, alongside `IncrTalker`/`IncrDomain`.
- **[FEAT]** `webui_api.go` 1.13.0 — `ClientInfo.LastSeen`, exposed as
  `last_seen` on `/api/clients`; resolved via one tracker `Snapshot()` rather
  than 2N sharded `Get()` calls.
- **[FEAT]** `webui_components.go` 1.33.0 / `script.js` 1.30.0 — "Last Seen"
  column, server-rendered then re-rendered on a 30s browser ticker so the age
  advances between the 60s polls; tooltip in the viewer's timezone.
- **[FEAT]** `stats.go` 3.1.0 — persisted as `client_last_seen` in
  `webui_stats.json`; `stats_hourly.go` 1.7.0 — pruned on the hourly tick.

#### Design notes (why not reuse what was already there)
- `topTracker` bins hits into **hourly** buckets and stores only counters, so the
  finest last-seen it could reconstruct is "some time during hour X" — useless
  for "is this device on the network right now".
- `arpSnap` reflects the kernel neighbour table: stale within minutes on a quiet
  host, expired wholesale, and blind to DoH/DoT/DoQ clients arriving off-link.
- `identSnap` is a static DHCP/hosts mapping with no notion of activity at all.

#### Hardening
- Hard ceiling of 8192 identities (256/shard) with oldest-first eviction. On a
  publicly reachable resolver the key space is attacker-controlled, so an
  unbounded map here would be a spoofed-source memory-growth vector; the cap
  turns it into a fixed ~1 MB cost and evicts the quietest keys first.
- Shard assignment is `maphash`-seeded per process — a flood cannot be steered
  onto one shard to concentrate lock contention or eviction pressure.
- The stamp is taken **after** the ACL and rate-limit gates (a source we refuse
  to serve must not be able to write into the tracker) and **before** the
  WebUI/policy/DGA/exfiltration blocks (a blocked device must still show a
  truthful last-seen instant instead of silently ageing out of the table).
- `Import()` applies both the age horizon and the entry ceiling, so a
  hand-edited or carried-over `webui_stats.json` cannot inflate memory at boot.
- Retention is 30 days, deliberately **not** `retentionHours()` — the default
  24h graph window would evict a device that last spoke 25 hours ago, which is
  exactly the record the column exists to show.
...
- 1.12.0 `webui_api.go` — O(N*M) nested MAC scan removed from `getKnownClients`;
  single MAC-keyed index, O(N+M).
- 1.11.0 `webui_api.go` — O(N^2) eviction scan hoisted out of the
  `handleApiClientsBulkBlock` loop.
- 1.10.0 `webui_api.go` — `handleApiClientsBulkBlock` added.
