...
Changes:
  2.46.0 - [FEAT] Added `MaxSecureUpstreams` field to `UpstreamGroup`, populated 
           from `server.max_secure_upstreams` at init time. Replaces the 
           previously hardcoded fan-out cap used by the "secure" strategy.
  2.45.0 - [FEAT] Added `echDomain` to natively support distinct DDR discovery 
           domains mapped dynamically from `?echdomain=` upstream configurations.
...
type UpstreamGroup struct {
	...
	// MaxSecureUpstreams caps how many servers within this group are queried
	// simultaneously by the "secure" strategy for consensus validation.
	// Populated from cfg.Server.MaxSecureUpstreams at init time (init_upstreams.go).
	// Defaults to 5 if left unset (0) — see exchangeSecure() in upstream_race.go.
	MaxSecureUpstreams int
	Servers           []*Upstream
...
type Upstream struct {
	...
	// echDomain holds the explicit domain name to query for discovering ECH configurations natively.
	// Bypasses the default DDR SVCB target organically if provided.
	echDomain string
