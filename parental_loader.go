...
Changes:
  1.24.0 - [PERF] Introduced a rigid 3-hour Local Freshness TTL (`LastFetch`) natively 
           across all remote extraction engines. Bypasses HTTP polling entirely if the 
           payload was successfully validated within the 3-hour horizon, completely 
           eliminating start-up latency and rate-limiting from strict CDNs.
...
type catListHeaders struct {
	...
	LastFetch    int64  `json:"last_fetch"` // Unix timestamp of last successful check/fetch
}
		...
		for _, src := range cc.ListURLs {
			if isSourceURL(src) {
				...
				if resp.StatusCode == http.StatusNotModified {
					...
					// Bound strictly to 50MB + 1 to detect stream overflows reliably natively.
