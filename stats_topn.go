...
Changes:
  1.9.0 - [PERF] Hoisted `time.Now().Unix()` evaluations organically outside of 
          Mutex locks in the `Add` hot-path. Massively minimizes lock contention 
          during extreme tracker floods natively.
  1.8.0 - [PERF] Bypassed memory allocations natively within `RecordBlockEvent` 
          by organically halting `recordRecentBlock` buffer execution when 
          `Search Domain Leak Prevention` is administratively disabled.
  1.7.0 - [REFACTOR] Injected the `RcodeStr` formatting capability natively to securely 
          standardize RCODE telemetry injections via the centralized helper.
  1.6.0 - [REFACTOR] Introduced `RecordBlockEvent` to atomically combine 
          `IncrBlockedDomain` and `recordRecentBlock` telemetry actions. 
          Simplifies distributed interception pipelines by centralizing 
          blocking analytics natively.
...
// RecordBlockEvent atomically records a domain block for both the global Top-N
// analytical tracker and the localized search-domain leak prevention buffer.
// Completely centralizes interception telemetry organically.
func RecordBlockEvent(clientIP, domain, reason string) {
	IncrBlockedDomain(domain, reason)
	if searchDomainLeakPrevention {
		recordRecentBlock(clientIP, domain, reason)
	}
}
