...
Changes:
  ...
  1.31.0 - [SECURITY/FIX] Guarded the outbound DoQ length prefix against uint16
           ...
           have announced its length modulo 65.536 and written the full body
           behind it, desynchronising the upstream's read offset for the rest of
           the connection. Far less likely to trigger than the response path
           (server.go 1.39.0) since an outbound query is a question plus a
           bounded OPT record — but "unlikely" is not a bound, and the EDNS0
           padding block on this path adds bytes. Oversized queries are now
           refused as an ordinary upstream failure, so the racing engine simply
           falls through to the next server and the client is still served.
           Header `Updated:` field also normalized to the mandated
           `Last Updated:` form.
  1.30.0 - [PERF/FIX] Eradicated a persistent slice memory leak natively within 
           the `takeStreamConn` multiplexing pool. The array now explicitly clears 
           pointers (`conns[len(conns)-1] = nil`) prior to array truncation, guaranteeing 
           that the Garbage Collector (GC) can organically reclaim discarded `streamConnEntry` 
           structures and socket interfaces during severe cache-miss floods natively.
...
func (u *Upstream) takeStreamConn(addr string) *dns.Conn {
	...
	for len(conns) > 0 {
		...
		// [PERF/FIX] Explicitly clear the pointer array mapping natively to prevent 
		// Memory Leaks. Ensures the Garbage Collector (GC) can safely reclaim 
		// the discarded socket structures once the slice boundary truncates.
...
func (u *Upstream) doqStreamExchange(ctx context.Context, conn *quic.Conn, req *dns.Msg) (*dns.Msg, error) {
	...
	// ── [SECURITY/FIX 1.31.0] 16-bit length-prefix guard (S-C) ───────────────
	//
	// RFC 9250 frames DoQ messages with a two-octet big-endian length, capping
	// a single message at 65.535 bytes. The conversion below was an unguarded
	// uint16(len(packed)), so an oversized query would have announced its
	// length modulo 65.536 and then written the full body behind it — leaving
	// the upstream reading our next query from the wrong offset for the rest of
	// the connection's life.
	//
	// This direction is far less likely to overflow than the response path
	// (server.go 1.39.0): outbound queries are a question plus an OPT record,
	// and prepareForwardQuery's EDNS0 padding is bounded. But "unlikely" is not
	// a bound, the padding block and any future EDNS0 option both add bytes on
	// this path, and a silently corrupted upstream stream is one of the harder
	// faults to diagnose after the fact.
	//
	// Refusing the dial is the correct action here, unlike on the response side:
