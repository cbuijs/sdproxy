...
Changes:
  1.30.0 - [FIX] Shipped the browser half of the "Last Seen" column, which
           version.go 1.449.0 documented as delivered but which was never
           actually written — this file remained at 1.29.0 while the server side
           moved to a five-column table.
           Three concrete defects, all user-visible:
             1. refreshClientsUI() emitted FOUR <td> per row (IP, MAC, Name,
                Action) against buildClientsHTML()'s five-column header. The
                server-rendered first paint was correct; the moment the 60s poll
                fired — or the operator hit refresh, or blocked/unblocked
                anyone — the Last Seen column vanished and the Action button
                slid under the "Last Seen" heading. The feature worked exactly
                once per page load.
             2. The empty-state placeholder still used colspan="4" where the Go
                builder had moved to colspan="5".
             3. No age ticker existed at all, so a label rendered at page load
                sat frozen until the next full navigation.
           Added fmtLastSeenAgo() (thresholds and wording mirroring
           stats_lastseen.go byte for byte, so the label never jumps when JS
           takes over from the server render), lastSeenCell(), and a 30s
           tickLastSeen() pass driven off the data-ts epoch. The ticker is bound
           to tbody rather than to the optional refresh button, and fires once
           immediately so the server-timezone tooltip from the first paint is
           upgraded to the viewer's local time without a refetch.
  1.29.0 - [FEAT] Bonded logic to explicitly map bulk-unknown assignments. 
           Permits dynamic evaluation of Privacy MACs/unnamed nodes to 
           streamline mass block/unblock executions.
  1.28.0 - [FEAT] Integrated dynamic keystroke filtering (`clients-filter`) organically 
           into the Client Tracking GUI. Instantly bounds the client array array 
           without incurring redundant DOM reloads.
  1.27.0 - [FEAT] Injected physical visual badges dynamically across the JSON 
           array extraction sequence to immediately highlight structurally 
           flagged MAC addresses natively.
  1.26.0 - [FEAT] Integrated the dynamic `Known Clients & Blocking` module. 
           Binds JSON API fetching directly to the GUI rendering sequence to 
           present comprehensive network nodes and manipulate explicit blocks seamlessly.
*/
  ...
  fetch('/api/set',{method:'POST',body:fd})
    ...
    .then(function(d){
      if(d.ok){
        ...
        if (window.refreshGroupsUI) {
            setTimeout(window.refreshGroupsUI, 400);
        } else {
  ...
  Object.keys(activeTimers).forEach(function(g) {
      ...
      if (t && t.expires_at > 0 && t.expires_at <= now) {
          if (!expiredNotified[g]) {
              ...
              if (window.refreshGroupsUI) {
                  setTimeout(window.refreshGroupsUI, 1500);
              } else {
...
// ── Section 1.5: Dynamic Group Refresh ────────────────────────────────────

// refreshGroupsUI dynamically reloads the structural Group/Device HTML array natively.
// Accepts an `isBackground` boolean flag. When true, the visual loading spinner 
// is suppressed to prevent distracting the user during automatic interval ticks.
window.refreshGroupsUI = function(isBackground) {
    var btnDesk = document.getElementById('btn-refresh-groups');
    var btnMob = document.getElementById('btn-refresh-groups-mob');
    
    if(isBackground !== true) {
        if(btnDesk) btnDesk.classList.add('spin');
        if(btnMob) btnMob.classList.add('spin');
    }

    // Capture states of elements natively
    var openGroups = {};
    document.querySelectorAll('#desk-groups-tbody details[data-group], #mob-groups-cards details[data-group]').forEach(function(d) {
        if(d.open) openGroups[d.dataset.group] = true;
    });
    var openDevs = {};
    document.querySelectorAll('#desk-groups-tbody details.dev-section, #mob-groups-cards details.dev-section').forEach(function(d) {
        if(d.open && d.dataset.device) openDevs[d.dataset.device] = true;
    });

    fetch('/api/groups')
        .then(function(r) { return r.json(); })
        .then(function(d) {
            var tbody = document.getElementById('desk-groups-tbody');
            if(tbody && d.desktop) tbody.innerHTML = d.desktop;
            
            var cards = document.getElementById('mob-groups-cards');
            if(cards && d.mobile) cards.innerHTML = d.mobile;

            // Re-apply states identically
            document.querySelectorAll('#desk-groups-tbody details[data-group], #mob-groups-cards details[data-group]').forEach(function(d) {
                if(openGroups[d.dataset.group]) d.open = true;
            });
            document.querySelectorAll('#desk-groups-tbody details.dev-section, #mob-groups-cards details.dev-section').forEach(function(d) {
                if(d.dataset.device && openDevs[d.dataset.device]) d.open = true;
            });

            if(btnDesk) btnDesk.classList.remove('spin');
            if(btnMob) btnMob.classList.remove('spin');
        })
        .catch(function() {
            if(btnDesk) btnDesk.classList.remove('spin');
            if(btnMob) btnMob.classList.remove('spin');
            // Only alert on explicit manual refreshes
            if(isBackground !== true) {
                var toast = document.getElementById('sdp-toast');
                if (toast) {
                    clearTimeout(window.toastT);
                    toast.textContent = 'Failed to refresh groups';
...
if(document.getElementById('btn-refresh-groups')) {
    document.getElementById('btn-refresh-groups').addEventListener('click', function() { window.refreshGroupsUI(false); });
    // [UI] Initiate an organic 60-second automatic background ticker to silently sync states
    setInterval(function() { window.refreshGroupsUI(true); }, 60000);
}
if(document.getElementById('btn-refresh-groups-mob')) {
    document.getElementById('btn-refresh-groups-mob').addEventListener('click', function() { window.refreshGroupsUI(false); });
}
...
(function(){
  ...
  var fields={
    ...
    'st-queries':'total_queries', 'st-queries-sub':null,
    'st-cache':'cache_hit_rate', 'st-cache-sub':null
  };
    ...
    fetch('/api/stats')
      ...
      .then(function(d){
        ...
        Object.keys(fields).forEach(function(id){
          ...
          } else if(id==='st-queries-sub'){
            el.textContent = 'Fwd: ' + (d.forwarded || 0) + ' | Blk: ' + (d.blocked || 0);
          } else if(id==='st-cache-sub'){
            var fill = d.cache_capacity > 0 ? (d.cache_entries + ' / ' + d.cache_capacity) : (d.cache_entries || 0);
            el.textContent = 'Hits: ' + (d.cache_hits || 0) + ' | Fill: ' + fill;
          } else if(key){
...
// ── Section 5: Clients & Blocking ────────────────────────────────────
(function(){
    var tbody = document.getElementById('clients-tbody');
    var refBtn = document.getElementById('btn-refresh-clients');
    var cFilt = document.getElementById('clients-filter');
    
    function escHtml(s){ return String(s).replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;').replace(/"/g,'&quot;'); }

    // ── [1.30.0] Last Seen rendering ──────────────────────────────────────
    //
    // Thresholds and wording are a deliberate byte-for-byte mirror of
    // fmtLastSeenAgo() in stats_lastseen.go. The server renders this column on
    // first paint and the browser re-renders it from data-ts thereafter, so any
    // divergence between the two would make the label visibly jump the moment
    // JavaScript takes over — the classic "it says 5m, then blinks to 4 min".
    //
    // ts is a raw unix epoch in SECONDS, exactly as /api/clients emits it. It
    // crosses the wire unformatted on purpose: ages then tick between the 60s
    // polls, and the absolute tooltip lands in the VIEWER's timezone rather
    // than the server's.
    function fmtLastSeenAgo(ts) {
        if(!ts || ts <= 0) return 'never';
        var delta = Math.floor(Date.now()/1000) - ts;

        // A negative delta means the stamp is in the future: a clock step (NTP
        // correction, VM resume, container host skew) landed between the server
        // writing it and the browser reading it. Clamp to "now" rather than
    ...
    // Builds the Last Seen cell. Kept as its own function because it is used by
    // the row renderer below and its output shape (the data-ts attribute in
    // particular) is what tickLastSeen depends on.
    //
    // ts == 0 is NOT an old timestamp — it means the client is known to ARP or
    // DHCP, or sits on the block list, but has never resolved anything through
    // sdproxy. Rendered muted so it reads as absence of data rather than as an
    // extreme age.
    function lastSeenCell(ts) {
        if(!ts || ts <= 0) {
            return '<span class="muted" title="No queries observed from this client">never</span>';
        }
        var abs = new Date(ts * 1000).toLocaleString();
        return '<span class="client-lastseen" data-ts="'+ts+'" title="'+escHtml(abs)+'">'+
               escHtml(fmtLastSeenAgo(ts))+'</span>';
    }

    // Re-renders every Last Seen label already in the DOM, without refetching.
    //
    // This is the whole reason the epoch is carried in data-ts. /api/clients is
    // polled once a minute; without this ticker a row would sit reading
    // "just now" for a full minute after the client went quiet, then jump. It
    // also upgrades the server-rendered first paint, whose title attribute is
    // formatted in the SERVER's timezone, to the viewer's local time.
    //
    // Cheap by construction: a querySelectorAll over at most a few hundred
    // spans plus a string compare per node, and it writes only when the label
    // has actually changed, so a quiet table causes no layout work at all.
    function tickLastSeen() {
        var nodes = document.querySelectorAll('#clients-tbody .client-lastseen');
        for(var i=0; i<nodes.length; i++) {
            var ts = parseInt(nodes[i].getAttribute('data-ts'), 10);
            if(!ts) continue;
            var next = fmtLastSeenAgo(ts);
            if(nodes[i].textContent !== next) nodes[i].textContent = next;
            nodes[i].setAttribute('title', new Date(ts * 1000).toLocaleString());
        }
    }

    function applyClientsFilter() {
        if(!tbody) return;
        var term = (cFilt ? cFilt.value.toLowerCase() : '');
        ...
        for(var i=0; i<trs.length; i++) {
            ...
            if(term === '') {
                ...
                tr.style.display = txt.indexOf(term) !== -1 ? '' : 'none';
            }
        }
    }

    if(cFilt) {
        cFilt.addEventListener('input', applyClientsFilter);
    }

    window.refreshClientsUI = function(isBackground) {
        if(!tbody) return;
        if(!isBackground && refBtn) refBtn.classList.add('spin');
        
        fetch('/api/clients')
            .then(function(r){ return r.json(); })
            .then(function(data){
                if(!data || data.length === 0) {
                    // [FIX 1.30.0] colspan tracks the header in
                    // webui_components.go buildClientsHTML: IP, MAC, Name,
                    // Last Seen, Action. It was left at 4 when the Last Seen
                    // column was added server-side, so the empty-state row
                    // under-spanned the table.
                    tbody.innerHTML = '<tr><td colspan="5" class="empty">No clients detected yet.</td></tr>';
                } else {
                    var html = '';
                    data.forEach(function(c) {
                        var trClass = c.is_blocked ? ' class="blocked-row"' : '';
                        var ipStr = c.ip ? escHtml(c.ip) : '<span class="muted">—</span>';
                        
                        var macStr = c.mac ? escHtml(c.mac) : '<span class="muted">—</span>';
                        if (c.is_rand_mac) {
                            macStr += ' <span class="reason-badge" title="Randomized / Private MAC Address">Rand</span>';
                        }
                        
                        var nameStr = c.name ? escHtml(c.name) : '<span class="muted">—</span>';
                        
                        var btnClass = c.is_blocked ? "btn-cancel" : "btn-clear";
                        var btnText = c.is_blocked ? "UNBLOCK" : "BLOCK";
                        
                        // [FIX 1.30.0] The Last Seen cell was missing entirely.
                        // This renderer emitted four cells against a five-column
                        // header, so the column vanished and the Action button
                        // slid under the "Last Seen" heading on the first poll
                        // after page load — i.e. the feature worked exactly once
                        // per page load and then broke.
                        html += '<tr data-id="'+escHtml(c.id)+'"'+trClass+'>'+
                            '<td style="font-family:monospace; text-align:left;">'+ipStr+'</td>'+
                            '<td style="font-family:monospace; text-align:left;">'+macStr+'</td>'+
                            '<td style="text-align:left;">'+nameStr+'</td>'+
                            '<td style="text-align:left; white-space:nowrap; font-size:0.85em; color:var(--text-dim);">'+lastSeenCell(c.last_seen)+'</td>'+
                            '<td><button class="client-block-btn '+btnClass+'" data-id="'+escHtml(c.id)+'" data-action="'+btnText+'" style="width:100%; padding: 6px; font-size:0.8em; border-radius:6px; font-weight:bold; cursor:pointer;">'+btnText+'</button></td>'+
                        '</tr>';
                    });
                    tbody.innerHTML = html;
                    applyClientsFilter(); // Preserve filter bounds organically
                }
    ...
    if(refBtn) {
        refBtn.addEventListener('click', function(){ window.refreshClientsUI(false); });
        setInterval(function(){ window.refreshClientsUI(true); }, 60000); 
    }

    // [1.30.0] Last Seen age ticker.
    //
    // Registered against tbody rather than refBtn: the refresh button is
    // optional chrome, while the column is always present, and hanging the
    // ticker off the button would silently disable it on any layout that omits
    // the button.
    //
    // 30s is half the /api/clients poll interval, so a label is never more than
    // half a minute stale — comfortably inside the one-minute granularity the
    // formatter renders at, which means the displayed value is always correct
    // rather than merely close.
    //
    // Fired once immediately as well as on the interval: the server-rendered
    // first paint carries a title attribute formatted in the SERVER's timezone,
    // and this upgrades it to the viewer's local time without waiting 30
    // seconds or forcing a refetch.
    if(tbody) {
        tickLastSeen();
        setInterval(tickLastSeen, 30000);
    }

    if(tbody) {
        tbody.addEventListener('click', function(e) {
            var btn = e.target.closest('.client-block-btn');
            if(!btn) return;
            ...
            fetch('/api/clients/block', {method: 'POST', body: fd})
                .then(function(r){ return r.json(); })
                .then(function(d){
                    if(d.ok) {
                        if(action === 'BLOCK') {
                            show('\u2713 Blocked ' + id, true);
                        } else {
                            show('\u2713 Unblocked ' + id, true);
                        }
                        window.refreshClientsUI(true);
                    } else {
                        show('\u2717 Failed: ' + (d.error || 'unknown'), false);
                        btn.disabled = false;
                        btn.style.opacity = '1';
                    }
                })
    ...
    // ── Bulk Actions for Unknown/Privacy MAC Clients ──
    var btnBulkUnknown = document.getElementById('btn-bulk-unknown');
    var selBulkUnknown = document.getElementById('bulk-unknown-action');
    if(btnBulkUnknown && selBulkUnknown) {
        btnBulkUnknown.addEventListener('click', function() {
            var action = selBulkUnknown.value;
            var fd = new URLSearchParams();
            fd.append('action', action);
            
            btnBulkUnknown.disabled = true;
            btnBulkUnknown.style.opacity = '0.5';

            fetch('/api/clients/bulk_block', {method: 'POST', body: fd})
                .then(function(r){ return r.json(); })
                .then(function(d){
                    if(d.ok) {
                        show('\u2713 Bulk ' + action + ' applied to ' + d.modified + ' clients', true);
                        window.refreshClientsUI(true);
                    } else {
                        show('\u2717 Failed: ' + (d.error || 'unknown'), false);
                    }
                    btnBulkUnknown.disabled = false;
                    btnBulkUnknown.style.opacity = '1';
                })
                .catch(function(){
                    show('\u2717 Network error', false);
                    btnBulkUnknown.disabled = false;
