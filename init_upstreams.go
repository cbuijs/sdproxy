...
func initUpstreams() {
	...
	for groupName, ugc := range cfg.Upstreams {
		...
		go func(gName string, gConf UpstreamGroupConfig) {
			...
			if logSystem {
				log.Printf("[INIT] Upstream group %q: %d server(s), strategy: %s (pref: %s, mode: %s, ignore_labels: %v, ecs: %s, isolated_cache: %v)", 
					gName, len(ups), strategy, pref, mode, gConf.IgnoreQnameLabels, grpECSAction, hasGrpClientName)
			}
			mu.Unlock()
		}(groupName, ugc)
	}
