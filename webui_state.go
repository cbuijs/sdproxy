...
Changes:
  1.6.0 - [SECURITY/FIX] Eradicated a persistent zombie goroutine organically. 
          The periodic override expiration ticker now explicitly listens for the 
          global `shutdownCh` multiplexer, ensuring clean resource teardowns natively.
  1.5.0 - [SECURITY/FIX] Upgraded `SaveGroupOverrides` to proactively monitor 
          and catch `os.WriteFile` errors. Abolishes edge-case anomalies where 
          faulty disk-allocations completely destroyed active temporary override states.
  1.4.0 - [SECURITY/FIX] Enforced strict lowercase normalization on group name keys 
          natively inside `GetGroupOverride()`, `SetGroupOverride()`, and `LoadGroupOverrides()`.
          Completely prevents casing mismatches and bypasses from capitalized API posts.
  1.3.0 - [LOGGING] Wired Web UI state transition logs securely into the 
          `logWebUI` toggle natively.
*/
...
func GetGroupOverride(name string) string {
	name = strings.ToLower(name) // Prevent bypasses via casing variants
