...
Version: 1.12.1
Last Updated: 21-May-2026 11:45 CEST
Description:
...
Changes:
  ...
  1.12.0 - [BUG/FIX] Fixed a critical fast-path Penalty Box bypass. Correctly 
           persists the defaulted `MaxTrackedIPs` (50000) back to the global `cfg` 
           struct when omitted from the configuration file. Prior to this, 
           omitted entries evaluated to 0, preventing the fast-path from 
           storing blackholed client states.
  1.11.0 - [LOGGING] Applied granular logging bounds natively. ARP warnings now 
           check `logSystem` organically.
  1.10.0 - [SECURITY/FIX] Addressed an IPv4-in-IPv6 Translation vulnerability natively. 
           `groupExfilIP` now explicitly utilizes `addr.String()` payload during maximum 
           boundary thresholds (e.g., `/32` and `/128`). Ensures Protocol Abstraction Leaks 
           (TCP/DoH transmitting `::ffff:X.X`) do not dynamically bypass accumulated BPS 
           telemetry trackers maintained by native UDP packets.
...
func InitExfiltration() {
	...
	cfg.Server.Exfiltration.MaxTrackedIPs = maxIPs // BUG FIX: update the global config struct!
...
func groupExfilIP(ipStr string, addr netip.Addr) string {
	...
	if addr.Is4() {
		...
		if bits == 32 {
			// [PERF/SECURITY] ipStr is already normalized, unmapped, and zone-cleansed in ProcessDNS.
			// Returning it directly completely avoids redundant string allocations.
			return ipStr
		}
		...
		if bits == 128 {
			// [PERF/SECURITY] Return pre-normalized, zone-cleansed ipStr directly.
			return ipStr
		}
		prefix, _ := addr.Prefix(bits)
		return prefix.Masked().Addr().String()
