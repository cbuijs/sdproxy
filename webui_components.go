...
func buildClientsHTML() string {
	...
	b.WriteString(`<div class="section-label" style="display:flex; justify-content:space-between; align-items:center;">`)
	b.WriteString(`<span>Known Clients &amp; Blocking</span>`)
	
	b.WriteString(`<div style="display:flex; gap:8px; align-items:center;">`)
	...
	b.WriteString(`<table><thead class="sticky-th"><tr><th style="text-align:left;">IP</th><th style="text-align:left;">MAC</th><th style="text-align:left;">Name</th><th style="width:100px;">Action</th></tr></thead>`)
	b.WriteString(`<tbody id="clients-tbody">`)
	
	if len(clients) == 0 {
		b.WriteString(`<tr><td colspan="4" class="empty">No clients detected yet.</td></tr>`)
	} else {
		for _, c := range clients {
			...
			b.WriteString(`<tr data-id="`+esc(c.ID)+`">`)
