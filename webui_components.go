...
Changes:
  1.34.0 - [SECURITY] Hardened HTML escaping. Two defects, one latent and one
           live.

           esc() did not encode the apostrophe. Every attribute this file emits
           is double-quoted today, so nothing is currently injectable — but the
           whole point of a central escaper is that a caller should not have to
           know which quote style the surrounding markup happens to use. One
           single-quoted attribute added anywhere in 955 lines of string
           concatenation turns esc() from a control into a liability, and the
           reviewer who adds it has no reason to suspect the helper is only
           conditionally correct. Now encodes ' as &#39;.

           buildTopNTable did NOT use esc(). It carried a private three-line
           escaper for the query-log filter attribute that handled &, < and "
           and silently omitted > and '. The value it escapes is
           attacker-influenced by construction: it is built from top-domain,
           top-blocked, top-NXDOMAIN and top-talker names, i.e. from QNAMEs any
           client on the network can put into the statistics simply by
           resolving them. The omitted > is not exploitable inside a quoted
           attribute on its own, but a second, weaker copy of a security
           control sitting next to the real one is precisely how a gap survives
           review. Deleted; the call site now uses esc() like every other.

           No behavioural change to correctly-rendered output: &#34; and &quot;
           are the same character, and the added entities only ever appear
           where a raw ' or > would otherwise have been emitted.
  1.33.0 - [UI] Added a "Last Seen" column to the Known Clients & Blocking
           table, rendered from ClientInfo.LastSeen (webui_api.go 1.13.0).

           Each cell carries a `data-ts` attribute holding the raw unix epoch
           alongside the server-rendered label. script.js 1.30.0 re-renders
           those cells from data-ts on a 30-second timer, so the age keeps
           advancing between the 60-second /api/clients polls without a single
           extra network round trip, and rewrites the tooltip into the viewer's
           own timezone rather than the server's.

           Rows with LastSeen == 0 render a muted "never" — a device present in
           the ARP/DHCP tables, or sitting in the block list, that has not
           actually resolved anything through sdproxy. That is a meaningfully
           different state from "seen a long time ago" and the column
           distinguishes them rather than showing a misleading age.

           The empty-table placeholder colspan moves 4 -> 5 to match.
  1.32.0 - [UI] Sourced and injected `bulk-unknown-action` tools directly into the 
           Client Tracking HTML array generation loop dynamically. Equips administrators 
           with robust granular toggles resolving Privacy nodes and Unidentified allocations.
  1.31.0 - [UI] Injected a real-time `clients-filter` search bounding box organically 
           into the Client Tracking GUI Engine. Vastly optimizes administrative targeting 
           operations across enormous DHCP top-talker architectures.
  1.30.0 - [UI] Dynamically suppress the rendering of the `Group overrides` structural 
           HTML entirely natively if no groups are configured. Prevents blank tables 
           and headers from cluttering the dashboard organically.
  1.29.0 - [UI] Injected dynamic visual tracking badges to explicitly highlight 
           randomized/private MAC addresses natively inside the HTML array 
...
// esc encodes a string for safe interpolation into HTML text nodes and into
// quoted attribute values.
//
// [SECURITY 1.34.0] The apostrophe was previously omitted, on the implicit
// assumption that every attribute in this file is double-quoted. That happens
// to be true right now. It is not an assumption a shared escaper is entitled to
// make: the helper is called from roughly forty sites across three files, and
// the next single-quoted attribute anyone writes would be injectable with no
// visible sign that anything was wrong. The escaper's contract is "safe in a
// quoted attribute", not "safe in a double-quoted attribute", and it now
// actually meets it.
//
// The five entities encoded here are the standard set. Order matters: & MUST be
// replaced first, or the ampersands introduced by the later substitutions would
// themselves be re-encoded into &amp;lt; and friends.
//
// Deliberately NOT html/template. That package escapes context-sensitively and
// would be the better tool for a rewrite, but this file assembles markup by
// string concatenation, and retrofitting it wholesale is a change of a very
// different size and risk to the one being made here. Numeric references are
// used for the quote characters (&#34;, &#39;) rather than named ones because
// &apos; is not defined in HTML 4 and the numeric forms are unambiguous
// everywhere.
...
func buildTopNTable(title, tbodyID string, rows []TopEntry, topN int, scrollStyle string) string {
	...
	if len(rows) == 0 {
		...
		for i, row := range rows {
			...
			// [SECURITY/FIX 1.34.0] Was a private inline escaper covering only
			// &, < and " — omitting > and '. The value is attacker-influenced
			// by construction: rawName comes from the top-domain, top-blocked,
			// top-NXDOMAIN and top-talker trackers, so any client on the
			// network can place a chosen string here simply by resolving it.
			// A second, weaker copy of a security control living beside the
			// real one is how the gap survived; there is now one escaper.
			fTerm := esc(filterStr)
			n := esc(rawName)
...
// ---------------------------------------------------------------------------
// Client Tracking GUI Engine
// ---------------------------------------------------------------------------

// buildClientsHTML orchestrates the physical HTML structure necessary to construct 
// the unified interactive client array seamlessly within the DOM natively.
func buildClientsHTML() string {
	clients := getKnownClients()
	
	var b strings.Builder
	b.WriteString(`<div class="section-divider"></div>`)
	b.WriteString(`<div class="section-label" style="display:flex; justify-content:space-between; align-items:center; flex-wrap:wrap; gap:8px;">`)
	b.WriteString(`<span>Known Clients &amp; Blocking</span>`)
	
	b.WriteString(`<div style="display:flex; gap:8px; align-items:center; flex-wrap:wrap;">`)
	
	// Bulk action for unknown
	b.WriteString(`<div style="display:flex; align-items:center; gap:4px; padding-right:8px; border-right:1px solid var(--border-light);">`)
	b.WriteString(`<select id="bulk-unknown-action" class="dur-sel" style="padding:2px 4px;"><option value="BLOCK">Block</option><option value="UNBLOCK" selected>Unblock</option></select>`)
	b.WriteString(`<button id="btn-bulk-unknown" class="pill" style="cursor:pointer; padding:3px 8px; font-size:0.8em; font-weight:600; border:none;" title="Apply to clients with randomized MACs or no hostname">Apply to Unknown</button>`)
	b.WriteString(`</div>`)
	
	b.WriteString(`<input type="text" id="clients-filter" class="clients-filter-input" placeholder="Filter clients...">`)
	b.WriteString(`<button id="btn-refresh-clients" class="ref-btn" title="Refresh Clients" style="width:22px; height:22px; display:flex; align-items:center; justify-content:center;">`)
	b.WriteString(`<svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="M21 12a9 9 0 1 1-9-9c2.52 0 4.93 1 6.74 2.74L21 8"></path><path d="M21 3v5h-5"></path></svg>`)
	b.WriteString(`</button></div></div>`)
	
	b.WriteString(`<div class="table-wrap" style="max-height: 400px; overflow-y: auto; margin-bottom: 20px;">`)
	b.WriteString(`<table><thead class="sticky-th"><tr><th style="text-align:left;">IP</th><th style="text-align:left;">MAC</th><th style="text-align:left;">Name</th><th style="text-align:left; width:120px;">Last Seen</th><th style="width:100px;">Action</th></tr></thead>`)
	b.WriteString(`<tbody id="clients-tbody">`)
	
	if len(clients) == 0 {
		// colspan tracks the header width above: IP, MAC, Name, Last Seen, Action.
		b.WriteString(`<tr><td colspan="5" class="empty">No clients detected yet.</td></tr>`)
	} else {
		for _, c := range clients {
			...
			if c.IsBlocked {
				btnClass = "btn-cancel"
				btnText = "UNBLOCK"
				trClass = ` class="blocked-row"`
			}
			
			ipStr := c.IP
			if ipStr == "" { 
				ipStr = `<span class="muted">—</span>` 
			} else { 
				ipStr = esc(ipStr) 
			}
			...
			nameStr := c.Name
			if nameStr == "" { nameStr = `<span class="muted">—</span>` } else { nameStr = esc(nameStr) }

			// [UI 1.33.0] Last Seen cell.
			//
			// Three pieces of state are emitted:
			//   • data-ts   — the raw unix epoch, so script.js can re-render the
			//                 age on its own timer and build a browser-local
			//                 absolute tooltip without re-fetching /api/clients.
			//   • title     — a server-timezone absolute fallback, used verbatim
			//                 until the client-side upgrade pass runs (and for
			//                 anyone browsing with JavaScript disabled).
			//   • body text — the relative age, identical in wording and
			//                 thresholds to the JS formatter so the label never
			//                 visibly jumps when the browser takes over.
			//
			// LastSeen == 0 is NOT an old timestamp: it means the client is
			// known to ARP/DHCP or to the block list but has never resolved
			// anything here. Rendered muted so it reads as an absence of data
			// rather than as an extreme age.
			var seenCell string
			if c.LastSeen <= 0 {
				seenCell = `<span class="muted" title="No queries observed from this client">never</span>`
			} else {
				abs := time.Unix(c.LastSeen, 0).Local().Format("02-Jan-2006 15:04:05")
				seenCell = `<span class="client-lastseen" data-ts="` + itoa64(c.LastSeen) + `" title="` + esc(abs) + `">` +
					esc(fmtLastSeenAgo(c.LastSeen)) + `</span>`
			}

			b.WriteString(`<tr data-id="`+esc(c.ID)+`"`+trClass+`>`)
			b.WriteString(`<td style="font-family:monospace; text-align:left;">`+ipStr+`</td>`)
			b.WriteString(`<td style="font-family:monospace; text-align:left;">`+macStr+`</td>`)
			b.WriteString(`<td style="text-align:left;">`+nameStr+`</td>`)
			b.WriteString(`<td style="text-align:left; white-space:nowrap; font-size:0.85em; color:var(--text-dim);">`+seenCell+`</td>`)
			b.WriteString(`<td><button class="client-block-btn `+btnClass+`" data-id="`+esc(c.ID)+`" data-action="`+btnText+`" style="width:100%; padding: 6px; font-size:0.8em; border-radius:6px; font-weight:bold; cursor:pointer;">`+btnText+`</button></td>`)
			b.WriteString(`</tr>`)
		}
	}
...
func computeStatus(name, sk string, grp GroupConfig) statusResult {
	...
	//
	// Note: the category name is pre-escaped here because renderBadge writes
	// statusResult.text verbatim — it is already HTML by the time it reaches
	// the badge. Category names come from the parental category maps, which are
	// operator- and feed-supplied rather than client-supplied, but escaping at
	// the point of construction is the only place the escaping decision is
	// locally verifiable.
...
// 
// If no groups are configured, it bypasses rendering entirely to conserve UI space natively.
