...
Changes:
  3.94.0 - [SECURITY/FIX] Passed `localPort` to `enforceSecurityGuards` dynamically
  ...
  3.92.0 - [SECURITY/FIX] Resolved a severe Custom Rules Routing Evasion vulnerability.
           ...
           identify and inherit explicit Group assignments, preventing routing bypasses
           where rules evaluated against the pre-override baseline state organically.
...
func ProcessDNS(w dns.ResponseWriter, r *dns.Msg, clientIP, protocol, sni, path string) {
	...
	if enforceSecurityGuards(w, r, q, originalQNameTrimmed, clientIP, clientAddr, clientMAC, clientName, clientID, protocol, localPort) {
		return
	}
	...
	// proactively resolve the FINAL state key and group BEFORE evaluating Custom Rules and RRS.
	// Otherwise, Custom Rules would evaluate against the pre-override group and completely miss explicit assignments.
	if clientRouteMatched && clientRoute.ClientName != "" && clientRoute.ClientName != clientName {
		effectiveClientName := clientRoute.ClientName
		effectiveClientNameLower := strings.ToLower(effectiveClientName)
		sk, clientGroup = ResolveStateKeyAndGroup(clientMAC, clientIP, clientAddr, effectiveClientName, effectiveClientNameLower, sni, sniLower, path, pathLower)
	}
