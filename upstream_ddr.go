...
Changes:
  2.51.0 - [SECURITY/FIX] Guarded the DDR DoQ probe's length prefix against
           ...
           This particular site is defensive rather than reactive: a DDR probe
           is a locally constructed SVCB question and cannot realistically
           approach 64 KB. It is fixed anyway because leaving one of three
           identical bugs in place is how it becomes the one nobody remembers,
           the day the probe grows an EDNS0 option.
			...
			case "doq":
				...
				if err == nil {
					...
					if err == nil && len(fwdBytes) > 65535 {
						...
						//
						// Same class of defect as server.go 1.39.0 and
						// upstream_net.go 1.31.0: the PutUint16 below wraps
						// modulo 65.536 on an oversized payload and then writes
						// the full body, leaving the peer parsing our stream
						// from the wrong offset.
						//
						// A DDR probe is a locally constructed SVCB question and
						// realistically cannot approach 64 KB, so this guard is
						// defensive rather than reactive. It is here anyway
						// because the three sites share one bug and fixing two
						// of them invites the third to be "the one nobody
						// noticed" the day the probe grows an EDNS0 option.
