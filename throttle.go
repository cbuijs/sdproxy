...
File:    throttle.go
Version: 2.5.0
Last Updated: 24-Jul-2026 16:40 CEST
...
Changes:
  2.5.0 - [DEAD-CODE/BLOAT] Removed the trailing `var _ atomic.Int64` "atomic
          import guard" and its comment. The guard was redundant: sync/atomic is
          already used natively by the throttler struct fields (activeQueries,
          activeUpstream, queryLimit, upstreamLimit are atomic.Int32; queriesTotal,
          upstreamCalls, droppedQueries, droppedUpstream are atomic.Int64), so the
          import is satisfied by real usage. The import itself is retained; only the
          dead blank declaration was excised. Behaviour-preserving. Header date field
          normalized from `Updated:` to the mandated `Last Updated:` form.
  2.4.0 - [SECURITY/FIX] Eradicated a persistent zombie goroutine organically.
          The periodic pressure monitor loop now explicitly bounds execution 
          against the global `shutdownCh` to abort safely during structural reloads.
  2.3.0 - [LOGGING] Managed initialization and active pressure diagnostic events 
          natively via the `logSystem` toggle.
