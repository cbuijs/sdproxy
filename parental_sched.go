...
Version: 1.3.0
Updated: 10-May-2026 09:01 CEST
...
Changes:
  1.3.0 - [FEAT] Incorporated `cfg.Parental.TimeOffsetHours` dynamically into 
          `inAnyScheduleWindow` to enable administrators to actively shift 
          system timezone constraints natively for schedule bounds.
  1.2.0 - [SECURITY/FIX] Hardened the `inAnyScheduleWindow` mechanic to explicitly 
          invoke `time.Now().Local()` securely. This guarantees that parental scheduling 
          blocks evaluate perfectly against the localized time boundaries of the host OS, 
          neutralizing bypass capabilities or false-positives caused by strictly configured 
          UTC environments natively.
  1.1.0 - [FIX] Resolved a severe scheduling interpretation flaw. 
          The documented `weekdays` and `weekend` string macros were missing 
          from the `dayNamesMap` evaluation loop. They are now natively translated 
          into `mon-fri` and `sat,sun` primitives before splitting.
*/
...
func inAnyScheduleWindow(schedule []string) bool {
	// [SECURITY/FIX] Explicitly force the evaluation against `.Local()` natively to 
	// guarantee OS boundary rules are honored securely over strict UTC environments.
	// [FEAT] Apply user-configured time offset natively to adjust schedule bounds.
...
func parseScheduleDays(spec string, wd time.Weekday) bool {
	// [FIX] Resolve documented "weekdays" and "weekend" macros natively
	spec = strings.ToLower(spec)
	spec = strings.ReplaceAll(spec, "weekdays", "mon-fri")
	spec = strings.ReplaceAll(spec, "weekend", "sat,sun")
...
func inTimeRange(t time.Time, rangeStr string) bool {
	...
	// [FIX] Handle 23:59 boundary cleanly to include the final minute of the day natively
	if end == 1439 {
		end = 1440
	}
