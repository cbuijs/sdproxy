...
func StartServers(tlsConf *tls.Config) {
	...
	for _, addr := range filterListeners(cfg.Server.ListenDoQ) {
		addr := addr
		go func() {
			// [SECURITY/FIX] Maintain an atomic counter to track and limit active DoQ connections.
			// Bounding this natively per-listener ensures malicious clients cannot open tens of 
			// thousands of DoQ handshakes to overwhelm the router's RAM or FD limits.
			...
			for {
				...
				// [SECURITY/FIX] Enforce the maxTCP connection bounds against incoming 
				// QUIC connections natively. Prevents malicious actors from bypassing 
				// FD limits and flooding the router with unfulfilled DoQ connections.
				if activeDoQConns.Add(1) > int32(maxTCP) {
					activeDoQConns.Add(-1) // Release bound capacity immediately
					
					// Terminate the connection organically with Application NO_ERROR (0x0).
					// Prevents clients from retrying aggressively, while shedding the connection load.
				...
				currentConn := conn
				go func() {
					defer activeDoQConns.Add(-1) // Safe deferral guarantees connection limits prune cleanly upon closure
					handleDoQConnection(currentConn) // Pass natively directly bypassing invalid casts
				}()
			}
		}()
	}
