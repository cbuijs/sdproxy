...
Changes:
  1.7.0 - [CLEANUP] Pruned the unused `responseContainsNullIP` utility function organically. 
          Its evaluation logic was successfully inlined directly across `process_upstream.go`, 
          `process_cachehit.go`, and `process_local.go` to eliminate structural execution bounds natively.
  1.6.0 - [PERF] Eradicated heap allocations in `applyAnswerSort` natively. 
          Utilizes fixed-size stack buffers for A/AAAA array slicing and ordering 
          operations. Massively drops Garbage Collection (GC) pressure when 
          permutating DNS responses.
...
//
// [PERF 1.6.0] Deploys fixed-size 16-element stack buffers to organically bypass 
// slice GC allocations. Safe fallback applies standard `make` sizing if the limits are reached.
func applyAnswerSort(msg *dns.Msg, method string) bool {
	...
	// [PERF] Zero-allocation arrays effectively absorb 99% of normal responses
	var aRecordsBuf [16]rrItem
	var aaaaRecordsBuf [16]rrItem
	aRecords := aRecordsBuf[:0]
	aaaaRecords := aaaaRecordsBuf[:0]
	...
	processGroup := func(group []rrItem) {
		...
		var sortBuf [16]dns.RR
		var sorted []dns.RR
		if len(group) <= 16 {
			sorted = sortBuf[:len(group)]
		} else {
