...
## 4. Secure Strategy Consolidation & RFC Compliance

The `"secure"` strategy provides advanced multi-resolver verification and secure fallback routing.

### Secure Consensus Mode

1. `sdproxy` fires the query to **all** upstreams in the group simultaneously.
2. It gathers and validates all returned responses under the matching execution rules:
   * **`mode: loose`** (Default): Failsafes and connection timeouts are ignored. Any RCODE other than `NOERROR`/`NXDOMAIN` is stripped from evaluation.
   * **`mode: strict`**: Requires 100% successful, uncorrupted response parity. Answer records are strictly verified to ensure identical resolving parameters.
3. Consensus payloads are validated to ensure no `NULL-IP` (ad-blocking sinkholes) or incompatible return codes are mixed.

### Payload Preference

* **`fastest`** (Default): Delivers the response that returned first with the lowest latency.
* **`ordered`**: Returns the response belonging to the primary upstream listed in the group's configuration array.
* **`consolidate`**: Merges all distinct, non-duplicate answers returned by all valid upstreams. This merges regional CDN IP mappings, geographical A/AAAA records, and multi-resolver configurations into a single consolidated record set for optimal client-side route redundancy.

#### The CNAME Consolidation Paradox & Automatic Flattening
Under **RFC 2181**, having multiple conflicting CNAME records for a single owner name (e.g. `www.example.com CNAME cdn-east.com` and `www.example.com CNAME cdn-west.com`) is strictly prohibited. If multiple upstream resolvers return different regional CNAME chains, presenting them as-is to a client violates protocol specifications and causes strict stub resolvers to fail or discard the query.

To preserve complete compliance and ensure seamless routing:
* Selecting `preference: "consolidate"` in an active secure group will **automatically force-enable `server.flatten_cname: true`** globally at boot time.
* Before the merged payload is written to the client, sdproxy transparently resolves and collapses the conflicting CNAME chains, delivering a single RFC-compliant set of direct `A` and `AAAA` IP addresses.

## 5. Initialization & Bootstrapping (Phase 1)
