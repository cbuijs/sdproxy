...
File:    dga.go
Version: 1.13.0
Last Updated: 10-Sep-2026 11:29 CEST
...
Changes:
  1.13.0 - [UPDATE] Updated SafeDomains with messengers and linkedin.
...
func AnalyzeDGA(fullDomain, domainCore string) float64 {
	...
	if len(fullDomain) > 0 {
		...
		if len(fd) > 0 {
			lastByte := fd[len(fd)-1]
			for _, safe := range dgaSafeDomains {
				if len(safe) > 0 && safe[len(safe)-1] != lastByte {
					continue
				}

				if strings.HasSuffix(fd, safe) {
					if len(fd) == len(safe) || fd[len(fd)-len(safe)-1] == '.' {
						return 0.0
					}
				}
			}
