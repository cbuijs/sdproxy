...
Changes:
  1.12.0 - [SECURITY/FIX] Restored strict raw 32-byte X25519 ECH private seed ingestion. 
           Replaced aggressive `bytes.TrimSpace` operations with precise structural 
           newline truncations natively. Ensures keys legitimately terminating in ASCII 
           whitespace characters (e.g., `0x20`) are not catastrophically shortened and 
           invalidated during configuration mapping.
  1.11.0 - [LOGGING] Bound initialization diagnostics to `logTLS` toggle gracefully.
...
func setupTLS() *tls.Config {
	...
	if cfg.Server.ECHKey != "" {
		...
		if block != nil {
			...
			rawKey := keyBytes
			// [SECURITY/FIX] Securely truncate trailing newline padding from text editors 
			// without invoking arbitrary whitespace trims (`bytes.TrimSpace`) which permanently 
			// corrupt raw cryptographic payloads terminating organically in ASCII-whitespace bytes (e.g., 0x20, 0x0A).
			if len(rawKey) == 33 && rawKey[32] == '\n' {
				rawKey = rawKey[:32]
			} else if len(rawKey) == 34 && rawKey[32] == '\r' && rawKey[33] == '\n' {
				rawKey = rawKey[:32]
			}
			if len(rawKey) == 32 {
				echPrivBytes = rawKey
			} else {
				log.Fatalf("[FATAL] TLS: ECH key file must be either PEM-encoded PKCS8 or a strictly 32-byte raw X25519 seed (got %d bytes)", len(keyBytes))
			}
		}
