...
Changes:
  1.7.0 - [FEAT] Extended the hourly cascade prune to the new per-client
          last-seen tracker (stats_lastseen.go 1.0.0).

          Note the horizon differs deliberately from the Top-N cascade above it:
          the trackers are pruned to retentionHours() because they feed the
          activity graphs, whereas statClientSeen is pruned to its own
          clientSeenMaxAge (30 days). Tying the client table to the graph
          retention would evict a device that last spoke 25 hours ago on a
          default 24h window — exactly the record the "Last Seen" column exists
          to show. The tracker's hard entry ceiling, not this prune, is what
          bounds its memory; the prune only keeps long-departed devices from
          occupying slots that a live device might need.
  1.6.0 - [SECURITY/FIX] Eradicated a persistent zombie goroutine organically.
           The periodic hourly metrics ticker now actively monitors the global 
           `shutdownCh` to surrender memory safely during graceful terminations.
           Mitigated potential 100% CPU lockups by ensuring the `time.NewTimer` 
           evaluates a strictly positive floor boundary natively.
...
func runHourlyTicker() {
	...
	if sleepDur <= 0 {
		sleepDur = time.Second // Prevent instant spin-loop
	}
...
func recordHourSlot() {
	...
	// [FEAT 1.7.0] Independent horizon — see the 1.7.0 header note.
	statClientSeen.Prune(clientSeenMaxAge)
}
